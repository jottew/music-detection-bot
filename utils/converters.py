...
class URLObject:
    ...
    async def save(
            self,
            fp: BufferedIOBase | os.PathLike[typing.Any],
            *,
    ...
    async def to_file(self, *, session: aiohttp.ClientSession = None):
        return discord.File(BytesIO(await self.read(session=session)), self.name, spoiler=False)

class FileConverter(commands.Converter):
    async def convert(self, ctx: commands.Context, file: str = None) -> discord.Attachment:
        if file is None:
            if ctx.message.attachments:
                ...
                if ctx.message.reference.resolved.attachments:
                    ...
                    raise commands.MissingRequiredArgument(inspect.Parameter("file", inspect.Parameter.KEYWORD_ONLY))
            else:
                raise commands.MissingRequiredArgument(inspect.Parameter("file", inspect.Parameter.KEYWORD_ONLY))
        else:
